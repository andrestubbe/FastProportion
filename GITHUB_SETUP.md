...
git branch -M main
git add -A
git commit -m "feat: initial commit for fastproportion v0.1.0"
```

### 2. Connect Remote Repository
Once the repository is created on GitHub under `andrestubbe/fastproportion`:
```bash
git remote add origin https://github.com/andrestubbe/fastproportion.git
...
```bash
gh repo edit andrestubbe/fastproportion \
