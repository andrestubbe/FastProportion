# Building fastproportion from Source
...
- Uses `native\fastproportion.def` for JNI exports
- Outputs to `build\fastproportion.dll`

The Maven `pom.xml` will automatically pick up `build\fastproportion.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\fastproportion.def` file:

```def
LIBRARY fastproportion
EXPORTS
    Java_fastproportion_fastproportion_doSomethingNative
```
